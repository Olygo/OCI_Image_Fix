...
def authenticate_cloud_shell(authentication_errors):
    ...
    try:
        print(yellow(" => Attempting CloudShell authentication...\n"))
...
def authenticate_config_file(authentication_errors, config_file_path, config_profile):
    ...
    try:
        print(yellow(" => Attempting Config File authentication...\n"))

        config_file_path=path_expander(config_file_path)
        ...
        # Validate the config by trying to get the tenancy_name
        identity=oci.identity.IdentityClient(config=config, signer=signer)
        tenancy=identity.get_tenancy(config["tenancy"], retry_strategy=custom_retry_strategy).data
...
def authenticate_instance_principals(authentication_errors):
    ...
    try:
        print(yellow(f" => Attempting Instance Principals authentication...\n"))
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                print(yellow(f"\r => Checking connectivity to region: {region.region_name} - [attempt: {attempt+1}/{max_retries}]"),end=" "*50+"\r", flush=True)
                time.sleep(2)
                continue
            else:
                return region, False, e
...
def validate_region_connectivity(regions, config, signer):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        futures={
            executor.submit(check_region_connectivity, region, config, signer, custom_retry_strategy): region 
            for region in regions
            }

        # Process results as they complete
        for future in as_completed(futures):
            region, success, error= future.result()
            ...
            if success:
               ...
               # print(f"\r", end=" " * 50, flush=True)
                errors.append(error)
                error_code = getattr(error, "code", "check domain replication")
                print_info(red, "Region", "ignored", region.region_name)
                print_info(red, "Region", "status", region.status)
                print_info(red, "Region", "error", error_code)
                # log error.message

    if regions_validated:
        return regions_validated, errors
    else:
        print_error(
...
def get_compartment_list(identity_client, compartment_id):
    ...
    """
    Retrieve a list of active compartments within a given compartment using a Depth-First Search (DFS) approach.

    This function fetches all child compartments of a specified parent compartment and filters out inactive ones.
    It uses a multi-threaded approach to retrieve child compartments concurrently for efficient processing.
    """
    #print(yellow("\r  => Fetching tenancy compartments..."), end=" "*15+"\r", flush=True)
    all_compartments=[]
    ...
    try:
        ...
        print_error("An error occurred in --get_compartment_list--:", compartment_id, e.code, e.message)
        raise SystemExit(1)

    # Stack for DFS-like traversal
    stack=[top_level_compartment_response.data]

    def fetch_child_compartments(target):
        try:
            print(yellow(f"\r  => Fetching tenancy compartments {target.name}..."), end=" "*50+"\r", flush=True)
            child_compartment_response=oci.pagination.list_call_get_all_results(
                identity_client.list_compartments,
                target.id
            )
            return child_compartment_response.data
        except oci.exceptions.ServiceError as e:
            print_error("An error occurred in --child_compartment_response--:", compartment_id, e.code, e.message)
            return []

    with ThreadPoolExecutor(max_workers=10) as executor:
        while stack:
            # Fetch child compartments concurrently
            futures={executor.submit(fetch_child_compartments, target): target for target in stack}
            stack.clear()  # Clear the stack since we are processing them in futures

            for future in as_completed(futures):
                target=futures[future]
                child_compartments=future.result()  # This will block until the future is complete
                stack.extend(child_compartments)  # Add new compartments to process
                all_compartments.extend(child_compartments)  # Keep track of all compartments

    # Filter active compartments using list comprehension
    active_compartments=[compartment for compartment in all_compartments if compartment.lifecycle_state == "ACTIVE"]
